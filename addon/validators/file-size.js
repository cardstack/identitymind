...
const FileSize = BaseValidator.extend({
  validate(value, options, model, attribute) {
    ...
    if (options.minInKb && fileSize < (options.minInKb * 1024)) {
      return `The file must be at least ${options.minInKb} KB in size. Please try again.`;
    }
    if (options.minInMb && fileSize < (options.minInMb * 1024 * 1024)) {
      return `The file must be at least ${options.minInMb} MB in size. Please try again.`;
    }
    return true;
  }
});
